...
# Variant of suffixrank.sh that drives the divsufsort-based pipeline:
# init_dss + refine_dss replace init + refine. The merge/update/create_pairs/
# invert/verify binaries are unchanged.
...
then
    ...
    echo "  WORD_LENGTH: must be 1 (init_dss restricts this)"
...
if [[ "$WORD_LENGTH" != "1" ]]; then
    echo "init_dss only supports word_length=1 (got '$WORD_LENGTH')"
    exit 1
fi
...
START=$($DATE)
./init_dss "$INPUT_DIR" "$RANK_DIR" "$CHUNKS_DIR" "$CHUNK_SIZE" "$WORD_LENGTH"
STATUS=$?
...
do
    ...
    ./refine_dss ${RANK_DIR} ${TEMP_DIR} $CHUNKS $H $CHUNK_SIZE
    STATUS=$?
