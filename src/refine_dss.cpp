// refine_dss: drop-in replacement for `refine`, paired with init_dss.
//
// Because init_dss writes a true partial suffix array (not just a bucket sort),
// sa_buffer is already in lex order; positions sharing the same current_rank
// are sub-sorted by what follows. So refine_dss can produce RunRecords with a
// single linear scan over sa_buffer, emitting whenever (curr, next) changes.
// No grouping, no tsort, no write-back of sa_buffer.
...
static inline void emit_run(RunRecord rec, RunRecord *out_buf, int *out_count,
	...
	if (*out_count == capacity) {
		...
		*out_count = 0;
		if (TEST_PERFORMANCE) {
			fprintf(stderr, "refine_dss: runs_buffer flushed mid-chunk (cap=%d)\n", capacity);
		}
	}
	out_buf[(*out_count)++] = rec;
}

static int generate_local_runs_dss(char *rank_dir, char *runs_dir, int total_chunks,
	...
	// next_ranks loading: identical to refine.c.
	if (next_chunk_dist) {
...
int main(int argc, char **argv) {
	if (argc < 6) {
		puts("Run ./refine_dss <rank_dir> <runs_dir> <total_chunks> <h> <working_chunk_size>");
		return FAILURE;
	}
	...
	for (int chunk_id = 0; chunk_id < total_chunks; chunk_id++) {
		int result = generate_local_runs_dss(rank_dir, runs_dir, total_chunks, chunk_id, h,
		                                     working_chunk_size,
